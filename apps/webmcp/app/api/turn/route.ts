...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (existing) {
      ...
      return withSession(NextResponse.json({ ok: true, operation_id: normalized.operation_id, status: existing.status, idempotent: true }), id);
    }
    if (view.session.resolver_mode === 'bridge') {
      const saved = await saveBridgeIntent(id, { payloadDigest, intent: normalized });
      return withSession(NextResponse.json({ ok: true, operation_id: normalized.operation_id, status: saved.status, session_revision: saved.view.session.revision, next_status: 'waiting', idempotent: false }), id);
    }
    const saved = await saveResolvedTurn(id, { payloadDigest, intent: normalized });
    return withSession(NextResponse.json({ ok: true, operation_id: normalized.operation_id, status: 'resolved', outcome: saved.resolution.outcome, resolution: { summary: saved.resolution.summary, visible_consequences: saved.resolution.visibleConsequences }, session_revision: saved.view.session.revision, next_status: saved.view.session.status === 'complete' ? 'complete' : 'ready', idempotent: false }), id);
  } catch (error) {
