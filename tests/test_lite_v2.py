...
def campaign(tmp_path: Path) -> Path:
    ...
    shutil.copytree(LITE / "campaign", target)
    state = (target / "current_state.yaml").read_text(encoding="utf-8")
    state = state.replace("campaign_id: new_campaign", "campaign_id: test")
...
- Local role: innkeeper
- Independent project: save for travel
- Current mundane task: finish supper
- Pressure decision rule: close the inn before helping
- Misbelief or recurring mistake: trusts familiar faces too quickly
- Hard boundary: no violence in the inn
- Non-player obligation: pay the supplier tomorrow
- Voice rhythm: short practical sentences
- Social tactic: offer a small favor first
- Routine and availability: works until closing
- Next move if ignored: close the inn
- Evaluation trigger: closing bell
- Visible consequence channel: shuttered windows
...
""",
    ...
    dossier = target / "research_dossier.md"
    dossier.write_text(dossier.read_text(encoding="utf-8").replace("Status: `needed_pending`", "Status: `complete`"), encoding="utf-8")
    voices = target / "world_voices" / "index.json"
    voice_state = json.loads(voices.read_text(encoding="utf-8"))
    voice_state["continuity_revision"] = 3
    voices.write_text(json.dumps(voice_state), encoding="utf-8")
    return target
...
def test_pending_research_cannot_silently_lock_world(campaign: Path) -> None:
    ...
    _write(campaign / "world_truths.md", "# World Truths\n\n- Status: locked\n")
    assert "research_truth_locked_while_pending" in _rules(check_state.check_campaign(campaign))
...
def test_documented_arc_stat_policy_downgrades_budget_exceptions(campaign: Path) -> None:
    ...
    profile.write_text(profile.read_text(encoding="utf-8").replace("resolution_grounding: fictional", "resolution_grounding: numeric"), encoding="utf-8")
    path.write_text(text, encoding="utf-8")
    rules = _rules(check_state.check_campaign(campaign))
    assert "stat_level_max_exceeded" not in rules
    assert "stat_budget_custom" in rules
...
def test_pending_visual_review_blocks_ready_for_play(campaign: Path) -> None:
    # Legacy Markdown-only campaigns use this compatibility path. Current
    # visual_state transactions are exercised by test_workspace_contracts.
    (campaign / "visual_state.json").unlink()
...
""",
    ...
    visual.write_text("# Visual Style\n- Pending visual review: yes\n", encoding="utf-8")
